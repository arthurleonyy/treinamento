...
public class ContaRest {
  ...
  @RequestMapping(value = "/consultar-saldo/{agencia}&{numeroConta}", method = RequestMethod.GET, produces = {
      MediaType.APPLICATION_JSON_VALUE })
  public @ResponseBody ResponseEntity<Double> consultarSaldo(
      final @PathVariable String agencia,
  ...
  @RequestMapping(value = "/consultar-extrato/{agencia}&{numeroConta}", method = RequestMethod.GET, produces = {
      MediaType.APPLICATION_JSON_VALUE })
  public @ResponseBody ResponseEntity<List<OperacaoFinanceira>> consultarExtrato(
      final @PathVariable String agencia,
      final @PathVariable String numeroConta) {
    List<OperacaoFinanceira> extrato = contaService.obterOperacoesPorAgenciaENumeroConta(agencia, numeroConta);
