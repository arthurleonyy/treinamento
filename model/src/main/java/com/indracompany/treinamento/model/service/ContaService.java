...
public class ContaService extends GenericCrudService<Conta, Long, ContaRepository>{
	...
	public void saque(Conta conta, double valor, OperacaoExtrato operacao) {
		...
		if(operacao.getDescricao().equals("Saida")){
			
			extratoService.gerarExtrato(conta, operacao, (conta.getSaldo()+valor), valor, 
					(conta.getSaldo()),"Saque na conta.: "+conta.getNumeroConta());
			
		}else if(operacao.getDescricao().equals("Transferencia")){
			
			/* Extrato gerado no metodo de Transferencia, 
			o IF serve apenas para validar o OperacaoExtrato enviado no momento
	...
	public void deposito(Conta conta, double valor, OperacaoExtrato operacao) {
		...
		if(operacao.getDescricao().equals("Entrada")){
			
			extratoService.gerarExtrato(conta, operacao, (conta.getSaldo()-valor), valor, 
					(conta.getSaldo()), "Deposito na conta.: "+conta.getNumeroConta());
	...
	public void transferencia (Conta contaOrigem, Conta contaDestino, double valor, OperacaoExtrato operacao) {
		...
		extratoService.gerarExtrato(contaOrigem, OperacaoExtrato.TRANSFERENCIA_SAIDA, contaOrigem.getSaldo()+valor, valor, 
				contaOrigem.getSaldo(), "Transferencia Efetuada para a Conta.: "+contaDestino.getNumeroConta());
		
		this.deposito(contaDestino, valor, operacao);
		extratoService.gerarExtrato(contaDestino, OperacaoExtrato.TRANSFERENCIA_ENTRADA, contaDestino.getSaldo()-valor, valor, 
				contaDestino.getSaldo(), "Transferencia Recebida da Conta.: "+contaOrigem.getNumeroConta());
